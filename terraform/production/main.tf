...
locals {
  ...
  dclg_access_group_notification_settings = {
    enabled                     = true
    additional_recipient_emails = ["delta-notifications@communities.gov.uk", "DELTAadmin@communities.gov.uk", "DELTAStatSupport@communities.gov.uk"]
  }
}
...
module "auth_service" {
  ...
  ldap_config = {
    ...
    GROUP_CONTAINER_DN          = "OU=Groups,OU=dluhcdata,DC=dluhcdata,DC=local"
  ...
  mail_settings = {
    ...
    from_address     = "delta@datacollection.communities.gov.uk"
