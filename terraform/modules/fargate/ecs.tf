...
resource "aws_ecs_task_definition" "main" {
  ...
  container_definitions = jsonencode(concat([{
    ...
    var.enable_adot_sidecar ? [{
      ...
      image : "${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/ecr-public/aws-observability/aws-otel-collector:v0.49.0"
      command : [""]
      essential : true
      environment = [
        {
