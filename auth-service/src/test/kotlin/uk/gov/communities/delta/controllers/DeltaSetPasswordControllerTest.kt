...
class DeltaSetPasswordControllerTest {
    ...
    fun testSetPasswordPage() = testSuspend {
        testClient.get(
            "/set-password?userGUID=" + user.getGUID().toString().encodeURLParameter() + "&token=$validToken"
        ).apply {
    ...
    @Test
    fun testSetPasswordPageWithCNParameter() = testSuspend {
    ...
    fun testGetSetPasswordPageExpiredTokenSendsEmail() = testSuspend {
        testClient.get(
            "/set-password?userGUID=" + user.getGUID().toString().encodeURLParameter() + "&token=expiredToken"
        ).apply {
                assertEquals(HttpStatusCode.OK, status)
    ...
    fun testSetPasswordSuccess() = testSuspend {
        testClient.submitForm(
            url = "/set-password?userGUID=" + user.getGUID().toString().encodeURLParameter() + "&token=" + validToken,
            formParameters = correctFormParameters()
        ).apply {
            coVerify(exactly = 1) { setPasswordTokenService.consumeTokenIfValid(validToken, user.getGUID()) }
            coVerify(exactly = 1) { userAuditService.setPasswordAudit(user.getGUID(), any()) }
    ...
    fun testSetPasswordValidationError() = testSuspend {
        testClient.submitForm(
            ...
            coVerify(exactly = 0) { setPasswordTokenService.consumeTokenIfValid(validToken, user.getGUID()) }
            assertFormPage(bodyAsText())
    ...
    fun testPostSetPasswordExpiredToken() = testSuspend {
        testClient.submitForm(
            url = "/set-password?userGUID=" + user.getGUID().toString().encodeURLParameter() + "&token=" + expiredToken,
            formParameters = parameters {
                append("newPassword", validPassword)
    ...
    fun testPostResendActivationEmail() = testSuspend {
        testClient.submitForm(
            ...
            coVerify(exactly = 1) { setPasswordTokenService.consumeTokenIfValid(expiredToken, user.getGUID()) }
            coVerify(exactly = 1) { emailService.sendNotYetEnabledEmail(any(), any(), any()) }
            coVerify(exactly = 1) { setPasswordTokenService.createToken(user.getGUID()) }
            assertEquals(HttpStatusCode.OK, status)
    ...
    fun resetMocks() {
        clearAllMocks()
        coEvery {
            setPasswordTokenService.validateToken(validToken, user.getGUID())
        } returns PasswordTokenService.ValidToken(validToken, user.getGUID())
        coEvery {
            setPasswordTokenService.validateToken(expiredToken, user.getGUID())
        } returns PasswordTokenService.ExpiredToken(expiredToken, user.getGUID())
        coEvery {
            setPasswordTokenService.validateToken(invalidToken, user.getGUID())
        } returns PasswordTokenService.NoSuchToken
        coEvery { setPasswordTokenService.validateToken("", any()) } returns PasswordTokenService.NoSuchToken
        coEvery {
            setPasswordTokenService.consumeTokenIfValid(validToken, user.getGUID())
        } returns PasswordTokenService.ValidToken(validToken, user.getGUID())
        coEvery {
            setPasswordTokenService.consumeTokenIfValid(expiredToken, user.getGUID())
        } returns PasswordTokenService.ExpiredToken(expiredToken, user.getGUID())
        coEvery {
            setPasswordTokenService.consumeTokenIfValid(invalidToken, user.getGUID())
        } returns PasswordTokenService.NoSuchToken
